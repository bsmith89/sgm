...
if __name__ == '__main__':
    ...
    community = pd.read_table(community_path)[lambda x: x.community_id == community_id]
    community.set_index('genome_id', inplace=True)
    assert community.index.is_unique, f'Some genomes are repeated in the community table.'
    replicon = pd.read_table(replicon_path)[lambda x: x.genome_id.isin(community.index)]
    replicon.set_index('replicon_id', inplace=True)
    assert replicon.index.is_unique, 'Some replicons are found multiple times in the replicon table.'
    assert set(replicon.genome_id) == set(community.index), 'Not all community genomes are found in the replicon table.'
    ...
    replicon['sim_pread'] = replicon['sim_nucleotides'] / replicon['sim_nucleotides'].sum()
    replicon['sim_nreads'] = np.random.multinomial(nreads, replicon['sim_pread'])
    print(replicon[['genome_id', 'genbank_id', 'size', 'sim_nreads']], file=sys.stderr)

    print('Simulating metagenome reads.', file=sys.stderr)
    err_model = KDErrorModel(os.path.join(iss.__path__[0], 'profiles', ERR_MODEL_NAME))
